...
st.markdown("""
    ...
    /* Estilo para el Podio en el Sidebar */
    .podio-container {
...
if datos_excel:
    ...
    # --- SIDEBAR: FILTROS Y PODIO INTERACTIVO ---
    st.sidebar.header("Menú de Filtros")
    
    # Podio de Saldo Pendiente
    st.sidebar.markdown("### 🏆 Top 3 Saldo Pendiente")
    df_podio = df_global.sort_values(by="Saldo_K", ascending=False).head(3)
    
    iconos = ["🥇", "🥈", "🥉"]
    colores_podio = ["#FFD700", "#C0C0C0", "#CD7F32"]
    
    podio_html = '<div class="podio-container">'
    for i, (index, row) in enumerate(df_podio.iterrows()):
        podio_html += f"""
        <div class="podio-item">
            <span style="font-size: 1.2rem;">{iconos[i]}</span> 
            <b style="color: {colores_podio[i]};">{row['País']}</b><br>
            <span style="color: #546e7a; font-size: 0.9rem;">$ {row['Saldo_K']:,.1f} K USD</span>
        </div>
        """
    podio_html += '</div>'
    ...
    # KPIs y Gráficas Detalladas (Se mantiene igual que antes)
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(px.pie(df_sel, values=col_tot, names='Estado_Final', hole=0.5, title="Cartera por Estado ($)", color='Estado_Final', color_discrete_map=color_map_estados), use_container_width=True)
    with c2:
        ...
        st.plotly_chart(px.bar(df_audit, x='Cantidad', y='Tipo', orientation='h', title="Auditoría: Tipo de Documento", color='Tipo', color_discrete_map={**color_map_estados, "Vigente": "#43a047"}), use_container_width=True)
