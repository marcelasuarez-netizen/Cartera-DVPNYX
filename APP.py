...
if datos_excel:
    ...
    df_rank = pd.DataFrame(resumen_global)
    df_rank['Venta_K'] = df_rank['Venta_Total_USD'] / 1000
    df_rank['Saldo_K'] = df_rank['Saldo_USD'] / 1000

    # --- 3. SIDEBAR (BANDERAS + 4 DESPLEGABLES) ---
    st.sidebar.header("Menú de Filtros")
    
    # 1. Filtro País
    pais_sel = st.sidebar.selectbox("🚩 Seleccionar País:", hojas_paises)
    st.sidebar.markdown(f"<div style='text-align: center; margin-bottom: 20px;'><img src='{BANDERAS.get(pais_sel.upper(), '')}' width='65'><br><b style='font-size:1.3rem; color:#0d47a1;'>{pais_sel}</b></div>", unsafe_allow_html=True)

    # Carga de datos específica del país para el resto de filtros
    ...
    # 4. Filtro Cliente
    lista_clientes = ["Todos"] + sorted(list(df_sel[col_cli].dropna().unique().astype(str)))
    cli_sel = st.sidebar.selectbox("👤 Seleccionar Cliente:", lista_clientes)
    ...
    with col_g1:
        fig_v = px.bar(df_rank, x="País", y="Venta_K", title="Ventas en USD (K)", color="País", color_discrete_map=color_map)
