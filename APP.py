...
st.markdown("""
    ...
    .notas-container {
        ...
        border-left: 6px solid #ffc107; margin-top: 20px;
    }
...
if datos_excel:
    ...
    # --- 3. SIDEBAR (FILTROS + PODIO + BANDERAS) ---
    ...
    if puesto <= 3:
        ...
        msg = {1: "🥇 Líder del ranking", 2: "🥈 Desempeño destacado", 3: "🥉 Top 3 del período"}
        st.sidebar.success(msg[puesto])
    else:
        st.sidebar.markdown("<br>", unsafe_allow_html=True)
        st.sidebar.warning("Fuera del Top 3")
    ...
    with col_g1:
        fig_v = px.bar(df_rank, x="País", y="Venta_K", title="Ventas en USD (K)", color="País", color_discrete_map={"COLOMBIA":"#1565C0","GUATEMALA":"#4DD0E1","MEXICO":"#43A047","ECUADOR":"#FFB300"})
        fig_v.update_traces(texttemplate='<b>%{y:.1f} K</b>', textposition='outside', textfont_size=14)
        fig_v.update_layout(yaxis=dict(range=[0, df_rank['Venta_K'].max()*1.2]), showlegend=False)
    ...
    with col_g2:
        ...
        fig_s.update_layout(yaxis=dict(range=[0, df_rank['Saldo_K'].max()*1.2]))
        st.plotly_chart(fig_s, use_container_width=True)

    st.markdown("---")

    # --- 5. DETALLE POR PAÍS + FILTROS TIEMPO ---
    ...
    if col_año:
        año_f = st.sidebar.selectbox("📅 Año:", ["Todos"] + sorted(list(pd.to_numeric(df_sel[col_año], errors='coerce').dropna().unique().astype(int)), reverse=True))
        if año_f != "Todos": df_sel = df_sel[pd.to_numeric(df_sel[col_año]) == año_f]
    if col_mes:
        mes_f = st.sidebar.selectbox("📆 Mes:", ["Todos"] + [f"{m} - {MESES_NOMBRES.get(m)}" for m in sorted(list(pd.to_numeric(df_sel[col_mes], errors='coerce').dropna().unique().astype(int)))])
        if mes_f != "Todos": df_sel = df_sel[pd.to_numeric(df_sel[col_mes]) == int(mes_f.split(" - ")[0])]

    # Cálculo de estados y KPIs
    ...
    def cls_f(row):
        ...
        if pd.to_numeric(row.get(col_sal_d), errors='coerce') == 0: return "🔵 Pagada"
        f_v = pd.to_datetime(row.get(col_ven_d), errors='coerce')
        return "🔴 En mora" if pd.notnull(f_v) and f_v < hoy else "🟢 Al día"

    df_sel['Estado_Final'] = df_sel.apply(cls_f, axis=1)
    
    # KPIs Visuales
    s_t = pd.to_numeric(df_sel[col_sub_d], errors='coerce').fillna(0).sum()
    nc_v = abs(pd.to_numeric(df_sel[df_sel['Estado_Final']=="NC"][col_sub_d], errors='coerce').fillna(0).sum())
    v_net_k = ((s_t - nc_v) / TASAS_REF.get(str(df_sel.iloc[0].get('Moneda', 'USD')).upper(), 1)) / 1000
    ...
    # Gráficas de detalle y Tabla
    c1, c2 = st.columns(2)
    with c1: st.plotly_chart(px.pie(df_sel, values=col_tot_d, names='Estado_Final', hole=0.5, title="Cartera por Estado", color='Estado_Final', color_discrete_map={"🔵 Pagada":"#1e88e5","🔴 En mora":"#e53935","Anulada":"#757575","NC":"#8e24aa","🟢 Al día":"#43a047"}), use_container_width=True)
    with c2:
        df_a = df_sel['Estado_Final'].apply(lambda x: x if x in ["NC", "Anulada"] else "Vigente").value_counts().reset_index()
        st.plotly_chart(px.bar(df_a, x='count', y='Estado_Final', orientation='h', title="Auditoría Documentos", color='Estado_Final', color_discrete_map={"Vigente":"#43a047","NC":"#8e24aa","Anulada":"#757575"}), use_container_width=True)
