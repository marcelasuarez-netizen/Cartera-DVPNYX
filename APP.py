...
# --- ESTILO CSS (Fondo Azul Claro, Tarjetas Blancas y Letra Financiera) ---
st.markdown("""
    <style>
...
# --- 2. CARGA DE DATOS ---
st.title("📊 Sistema de Control Financiero: Auditoría y Cartera Unificada")
st.markdown("---")
...
if datos_excel:
    ...
    # --- 3. PROCESAMIENTO GLOBAL USD (Unificado por Saldo) ---
    resumen_global = []
    for p in hojas_paises:
        ...
        if c_tot in df_p.columns and c_sal:
            ...
            total_s_usd = pd.to_numeric(df_p[c_sal], errors='coerce').fillna(0).sum() / tasa
            
            resumen_global.append({
                "País": p, 
                "Venta_Total_USD": total_v_usd, 
                "Saldo_Pendiente_USD": total_s_usd
            })

    df_global = pd.DataFrame(resumen_global)
    col_g1, col_g2 = st.columns(2)
    with col_g1:
        st.plotly_chart(px.bar(df_global.sort_values("Venta_Total_USD", ascending=False), x="País", y="Venta_Total_USD", text_auto=',.0f', title="Ventas Totales por País (USD)", color="País").update_layout(template="plotly_white", paper_bgcolor='rgba(0,0,0,0)'), use_container_width=True)
    with col_g2:
        st.plotly_chart(px.bar(df_global.sort_values("Saldo_Pendiente_USD", ascending=False), x="País", y="Saldo_Pendiente_USD", text_auto=',.0f', title="Saldo Real Pendiente (USD)", color_discrete_sequence=['#e53935']).update_layout(template="plotly_white", paper_bgcolor='rgba(0,0,0,0)'), use_container_width=True)
    ...
    # Mapeo de columnas con columna SALDO obligatoria
    ...
    v_total = df_sel[~df_sel['Estado_Final'].isin(["NC", "ANULADA"])][col_tot].sum()
    saldo_total = df_sel[col_sal].sum()
    mora_total = df_sel[df_sel['Estado_Final']=='🔴 EN MORA'][col_sal].sum() # Mora basada en saldo

    k1.metric("Venta Bruta (Vig)", f"$ {v_total:,.2f}")
    k2.metric("SALDO PENDIENTE", f"$ {saldo_total:,.2f}")
    k3.metric("Monto en Mora", f"$ {mora_total:,.2f}")
    k4.metric("Recaudado (Estimado)", f"$ {v_total - saldo_total:,.2f}")
    dso_r = (saldo_total / v_total * 360) if v_total > 0 else 0
    k5.metric("DSO (Días)", f"{dso_r:.0f}")

    k_sub, k_iva, k_ret, k_cnt = st.columns(4)
    k_sub.metric("Subtotal", f"$ {df_sel[col_sub].sum():,.2f}")
    k_iva.metric("IVA", f"$ {df_sel[col_iva].sum():,.2f}")
    k_ret.metric("Retenciones", f"$ {df_sel['Total_Retenciones'].sum():,.2f}")
    k_cnt.metric("Documentos", f"{len(df_sel):,d} Und")
    ...
    with c_p:
        st.plotly_chart(px.pie(df_sel, values=col_tot, names='Estado_Final', hole=0.5, title="Cartera por Estado (Monto Total)", color='Estado_Final', color_discrete_map={"🔵 PAGADA": "#1e88e5", "🔴 EN MORA": "#e53935", "🟠 CRUCE": "#fb8c00", "🟢 AL DÍA": "#43a047", "NC": "#8e24aa", "ANULADA": "#757575"}).update_layout(paper_bgcolor='rgba(0,0,0,0)'), use_container_width=True)
    with c_a:
        df_audit = df_sel['Estado_Final'].apply(lambda x: x if x in ["NC", "ANULADA"] else "VIGENTE").value_counts().reset_index()
        st.plotly_chart(px.bar(df_audit, x='count', y='index', orientation='h', title="Auditoría: Tipo de Documento", color='index', color_discrete_map={"NC": "#8e24aa", "ANULADA": "#757575", "VIGENTE": "#43a047"}).update_layout(paper_bgcolor='rgba(0,0,0,0)', showlegend=False), use_container_width=True)
    ...
    st.error("Archivo no encontrado o ID incorrecto.")