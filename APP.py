...
st.markdown("""
    ...
    [data-testid="stMetricLabel"] { font-size: 0.85rem !important; color: #546e7a; }
...
# --- 2. CARGA DE DATOS ---
st.title("📊 Control Financiero: Auditoría y Cartera Unificada")
st.markdown("---")
...
if datos_excel:
    ...
    for p in hojas_paises:
        ...
        if c_tot in df_p.columns:
            ...
            total_v = pd.to_numeric(df_p[c_tot], errors='coerce').fillna(0).sum() / tasa
            total_s = pd.to_numeric(df_p.get(c_sal, 0), errors='coerce').fillna(0).sum() / tasa
            resumen_global.append({"País": p, "Venta_Total_USD": total_v, "Saldo_Pendiente_USD": total_s})

    df_global = pd.DataFrame(resumen_global)
    col_g1, col_g2 = st.columns(2)
    with col_g1:
        st.plotly_chart(px.bar(df_global, x="País", y="Venta_Total_USD", text_auto='.2s', title="Ventas Totales (USD)", color="País").update_layout(template="plotly_white", paper_bgcolor='rgba(0,0,0,0)'), use_container_width=True)
    with col_g2:
        st.plotly_chart(px.bar(df_global, x="País", y="Saldo_Pendiente_USD", text_auto='.2s', title="Saldo Pendiente (USD)", color_discrete_sequence=['#e53935']).update_layout(template="plotly_white", paper_bgcolor='rgba(0,0,0,0)'), use_container_width=True)
    ...
    df_sel['Auditoria_Simplificada'] = df_sel['Estado_Final'].apply(lambda x: x if x in ["NC", "ANULADA"] else "VIGENTE")

    # KPIs
    st.header(f"Gestión Detallada: {pais_sel}")
    k1, k2, k3, k4, k5 = st.columns(5)
    v_total = df_sel[~df_sel['Estado_Final'].isin(["NC", "ANULADA"])][col_tot].sum()
    s_total = df_sel[col_sal].sum()
    k1.metric("Venta Bruta", f"$ {v_total:,.2f}")
    k2.metric("SALDO PENDIENTE", f"$ {s_total:,.2f}")
    k3.metric("Monto en Mora", f"$ {df_sel[df_sel['Estado_Final']=='🔴 EN MORA'][col_sal].sum():,.2f}")
    k4.metric("DSO (Días)", f"{(s_total/v_total*360) if v_total>0 else 0:.0f}")
    k5.metric("Documentos", f"{len(df_sel):,d} Und")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(px.pie(df_sel, values=col_tot, names='Estado_Final', hole=0.5, title="Cartera por Estado ($)", 
                               color='Estado_Final', color_discrete_map={"🔵 PAGADA": "#1e88e5", "🔴 EN MORA": "#e53935", "🟠 CRUCE": "#fb8c00", "🟢 AL DÍA": "#43a047", "NC": "#8e24aa", "ANULADA": "#757575"}).update_layout(paper_bgcolor='rgba(0,0,0,0)'), use_container_width=True)
    
    with c2:
        # CORRECCIÓN DEL VALUE_COUNTS PARA EVITAR EL VALUEERROR
        df_audit = df_sel['Auditoria_Simplificada'].value_counts().reset_index()
        df_audit.columns = ['Tipo', 'Cantidad'] # Renombramos manualmente para estar seguros
        
        st.plotly_chart(px.bar(df_audit, x='Cantidad', y='Tipo', orientation='h', title="Auditoría: Tipo de Documento", 
                               color='Tipo', color_discrete_map={"NC": "#8e24aa", "ANULADA": "#757575", "VIGENTE": "#43a047"}).update_layout(paper_bgcolor='rgba(0,0,0,0)', showlegend=False), use_container_width=True)
