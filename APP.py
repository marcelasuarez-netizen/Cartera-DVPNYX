...
st.markdown("""
    ...
    /* Contenedor genérico para métricas personalizadas */
    .metric-custom {
        background-color: #ffffff;
        padding: 10px 15px;
        border-radius: 10px;
...
if datos_excel:
    ...
    monto_mora = df_sel[df_sel['Estado_Final']=='🔴 En mora'][col_sal].sum()
    ...
    with r1c0:
        st.markdown(f"""<div class="metric-custom">
            <p class="metric-custom-label">Conversión Dólares</p>
            <p class="metric-custom-value">$ {venta_neteada_usd_miles:,.2f} K</p>
    ...
    # --- MODIFICACIÓN: MONTO EN MORA CON ETIQUETA ROJA ---
    with r1c4:
        st.markdown(f"""<div class="metric-custom">
            <p class="metric-custom-label" style="color: #d32f2f; font-weight: bold;">Monto en mora</p>
            <p class="metric-custom-value">$ {monto_mora:,.2f}</p>
        </div>""", unsafe_allow_html=True)
