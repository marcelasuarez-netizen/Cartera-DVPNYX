...
if datos_excel:
    ...
    for p in hojas_paises:
        ...
        if c_tot in df_p.columns:
            ...
            def es_mora_global(row):
                ...
                if any(x in txt for x in ["PAGADA", "CRUCE"]) or pd.notnull(row.get('Fecha de Pago')): return False
            ...
            # Cartera consolidada (excluye anuladas)
            df_v = df_p[~df_p[c_est].str.contains("ANULADA|CANCELADO", case=False, na=False)]
            resumen_global.append({
                "País": p, 
                "Cartera_Global_USD": df_v[c_tot].sum() / tasa,
                "Mora_USD": df_p[df_p['Es_Mora']][c_tot].sum() / tasa
            })
    ...
    with col_g2:
        fig_m = px.bar(df_global.sort_values("Mora_USD", ascending=False), x="País", y="Mora_USD", text_auto=',.0f', color_discrete_sequence=['#d32f2f'], title="Mora por País (USD)")
    ...
    rec = df_sel[df_sel['Estado_Final'].isin(['🔵 PAGADA', '🟠 CRUCE'])][col_tot].sum()
    dso = (df_sel[df_sel['Estado_Final'].isin(["🔴 EN MORA", "🟢 AL DÍA"])][col_tot].sum() / v_vig * 360) if v_vig > 0 else 0
    
    k1.metric("Cartera (Vig)", f"$ {v_vig:,.2f}")
    k2.metric("En Mora", f"$ {mora:,.2f}")
    k3.metric("Recaudado", f"$ {rec:,.2f}")
    k4.metric("DSO (Días)", f"{dso:.0f}")
    ...
    with c1:
        fig_p = px.pie(df_sel, values=col_tot, names='Estado_Final', hole=0.5, title="Cartera por Estado ($)",
                       color='Estado_Final', color_discrete_map={"🔵 PAGADA": "#1e88e5", "🔴 EN MORA": "#e53935", "🟠 CRUCE": "#fb8c00", "🟢 AL DÍA": "#43a047", "NC": "#8e24aa", "ANULADA": "#757575"})
    ...
    with c2:
        ...
        fig_a.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False)
        st.plotly_chart(fig_a, use_container_width=True)
    with c3:
        if col_ser in df_sel.columns:
            fig_s = px.bar(df_sel[col_ser].value_counts().reset_index(), x='count', y=col_ser, orientation='h', text_auto=',d', title="Mix de Servicios")
            fig_s.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig_s, use_container_width=True)

    st.dataframe(df_sel[[col_cli, col_ser, col_tot, 'Estado_Final']].sort_values(by=col_tot, ascending=False).style.format({col_tot: "{:,.2f}"}))
